function [res,resL2,qualMeasOut]=SIRT2(proj,geo,angles,niter,varargin)
...
V=computeV2(geo,angles,{angles},{1:length(angles)},'gpuids',gpuids);
...
for ii=1:niter
    ...
    % THIS TAKES MEMORY!
    if measurequality && ~any(strcmp(QualMeasOpts,'error_norm'))
    ...
    if nesterov
        ...
        ynesterov=res + V.*Atb(W.*(proj-Ax(res,geo,angles,'gpuids',gpuids)),geo,angles,'gpuids',gpuids);
        res=(1-gamma)*ynesterov+gamma*ynesterov_prev;
    else
        res=res+lambda*V.*Atb(W.*(proj - Ax(res,geo,angles,'gpuids',gpuids)),geo,angles,'gpuids',gpuids); % x= x + lambda * V * At * W^-1 * (b-Ax)
    end
    % ------------------------------------
