function Vinv=computeV2(geo,angles,alphablocks,orig_index,varargin)
if nargin>=6
...
if ~isfield(geo,'mode')||~strcmp(geo.mode,'parallel')
    for ii=1:length(alphablocks)
        ...
        %auxgeo=geo;
%         V(:,:,ii) = mean(Atb(ones(geo.nDetector(2),geo.nDetector(1),size(auxang,2),'single'),auxgeo,auxang,'gpuids',gpuids),3,"omitnan")+0.000001;
        V = Atb(ones(geo.nDetector(2),geo.nDetector(1),size(auxang,2),'single'),auxgeo,auxang,'gpuids',gpuids);
    end
    V(V<0)=0;
    V(isinf(V))=0;
    V(isnan(V))=0;
    V(imag(V) ~= 0) = 0;
    kernel = ones(5, 5, 5, 'single') / 125;
    V_avg = convn(V, kernel, 'same');  % 'same' mantém o tamanho original
    Vinv = 1./V_avg;
    Vinv(isinf(Vinv))=0;
    Vinv(isnan(Vinv))=0;
else
